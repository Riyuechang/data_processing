...
for novel_translation_file in tqdm_progress:
    tqdm_progress.set_description(novel_translation_file)
    ...
    re_translation_count = 0
    for chapter_index, chapter in  enumerate(translation_data):
        for content_index, content in enumerate(chapter["content"]):
            ...
            if len(jp_seg) != len(translation_seg):
                ...
                re_translation_count += 1

                vllm_add_request(
                    input_text=content["jp"].strip("\n"),
                    request_id=f"{chapter_index}:{content_index}#{newline}"
                )
        
    while True:
        if not re_translation_count:
            break
        ...
        for output in request:
            if output.finished:
                ...
                chapter_index, content_index = data_index.split(":")
                ...
                translation_data[int(chapter_index)]["content"][int(content_index)]["translation"] = output.outputs[0].text + add_newline

        if not llm_engine.has_unfinished_requests():
            break
    
    if re_translation_count:
        with open(f"{SAVE_DIR_PATH}/{novel_translation_file}", 'w', encoding='utf-8') as file:
            json.dump(translation_data, file, indent=4, ensure_ascii=False)
